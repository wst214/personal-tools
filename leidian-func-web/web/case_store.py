...
# 一级：规则 / 抑制 / 运行 / 接入 / 业务监测查询；因子、回测并入规则侧
CAPABILITIES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "name": "业务监测查询",
        "modules": [
            {"id": "device-monitor-query", "name": "列表与详情"},
        ],
    },
    {
        "id": "radar-frame-ingest",
        "name": "雷达回波接入",
        "modules": [
            {"id": "radar-frame-smoke", "name": "接入烟测"},
            {"id": "radar-frame-query", "name": "查询验收"},
            {"id": "radar-frame-e2e", "name": "全链路端到端"},
            {"id": "radar-frame-boundary", "name": "边界与隔离"},
        ],
    },
    {
        "id": "warn-rule",
...
def format_device_type_label(device: dict[str, Any]) -> str:
    """勾选展示名：中文名 + 协议类型编码，如「接地电阻 (03)」「SPD波形 (18)」。"""
    name = str(device.get("name") or device.get("id") or "").strip()
    codes = [str(c).strip().upper() for c in (device.get("typeCodes") or []) if str(c).strip()]
    if not codes:
        return name
    return f"{name} ({'/'.join(codes)})"


def device_has_hex_fixture(device: dict[str, Any]) -> bool:
    """可勾选设备是否具备全链路 HEX（含 SPD 多报文变体）。"""
    variants = device.get("variants") or []
    if variants:
        return all(bool(str(v.get("deviceHex") or "").strip()) for v in variants)
    return bool(str(device.get("deviceHex") or "").strip())


def device_has_biz_query(device: dict[str, Any]) -> bool:
    """勾选设备是否具备 biz 监测列表（含 SPD 仅摘要有 BFF）。"""
    if device.get("bizPath"):
        return True
    return any(bool(v.get("bizPath")) for v in (device.get("variants") or []))


def expand_device_run_units(device: dict[str, Any]) -> list[dict[str, Any]]:
    """一个勾选项 → 一条或多条执行单元（SPD 心跳/摘要各一条，monitorType 不同）。"""
    label = format_device_type_label(device)
    variants = device.get("variants") or []
    if not variants:
        return [
            {
                "selectableId": device["id"],
                "deviceType": device["id"],
                "deviceTypeName": label,
                "bizPath": device.get("bizPath") or "",
                "detailPath": device.get("detailPath") or "",
                "deviceHex": device.get("deviceHex") or "",
            }
        ]
        ...
        units.append(
            {
                "selectableId": device["id"],
                "deviceType": v["id"],
                "deviceTypeName": f"{label} · {vname}" if vname else label,
                "bizPath": (v.get("bizPath") if "bizPath" in v else device.get("bizPath")) or "",
                "detailPath": (v.get("detailPath") if "detailPath" in v else device.get("detailPath")) or "",
                "deviceHex": v.get("deviceHex") or "",
            }
        )
    return units
...
def list_device_types() -> list[dict[str, Any]]:
    """操作台勾选列表（按物理设备；SPD 合并为一项）。"""
    return [
        {
            "id": t["id"],
            "name": format_device_type_label(t),
            "shortName": t["name"],
            "typeCodes": list(t.get("typeCodes") or []),
            "hasBizQuery": device_has_biz_query(t),
        }
...
CAPABILITY_ALIASES = {
    ...
    "device-ingest-biz-query": "device-monitor",
}
...
LEGACY_MODULE_ALIASES = {
    ...
    "device-ingest-biz-query": "device-monitor-query",
    "device-monitor": "device-monitor-query",
    "lightning-ingest": "lightning-ingest-smoke",
    "radar-frame-ingest": "radar-frame-smoke",
}
...
# 设备解析能力：按物理设备勾选（对齐协议 typeId；SPD 心跳/摘要同属 type=18，合并为一勾选项）
# bizPath / detailPath：biz 监测 BFF；空表示无对外业务列表/详情（如 SPD 心跳）
# deviceHex：全链路注入用合法 HEX（CRC 有效）；含 variants 时各变体均须具备夹具
# typeCodes：协议设备类型编码（展示用）
DEVICE_TYPES: list[dict[str, Any]] = [
    {
        ...
        'detailPath': '/monitor/atmosphere/devices/${deviceId}',
    ...
    },
    {
        ...
        'detailPath': '/monitor/grounding/devices/${deviceId}',
    ...
    },
    {
        ...
        'detailPath': '/monitor/surge-current/devices/${deviceId}',
    ...
    },
    {
        ...
        'detailPath': '/monitor/ispd-pdu/devices/${deviceId}',
    ...
    },
    {
        ...
        'name': '避雷器在线监测',
        'typeCodes': ['14'],
        'bizPath': '/monitor/surge-monitor/surge-monitors?pageNum=1&pageSize=10',
        'detailPath': '/monitor/surge-monitor/devices/${deviceId}',
        'deviceHex': '5A4B0B14000000010001000A0E74B87E0D0A',
    },
    {
        'id': 'DISCONNECT_CARD',
        'name': '断接卡',
        'typeCodes': ['09'],
        'bizPath': '/monitor/disconnect-card/disconnect-cards?pageNum=1&pageSize=10',
        'detailPath': '/monitor/disconnect-card/devices/${deviceId}',
        'deviceHex': '5A4B0A090000000100010001724F3A0D0A',
    },
    {
        'id': 'REMOTE_TERMINAL',
        'name': '远程终端',
        'typeCodes': ['10'],
        'bizPath': '/monitor/remote-terminal/remote-terminals?pageNum=1&pageSize=10',
        'detailPath': '/monitor/remote-terminal/devices/${deviceId}',
        'deviceHex': '5A4B2C1000000001000101435C00003FC0000043A500003F73333342480000447A000000000000096004B0025801F4050D0D0A',
    },
    {
        'id': 'POWER_BOARD',
        'name': '电源控制板',
        'typeCodes': ['17'],
        'bizPath': '/monitor/power-board/power-boards?pageNum=1&pageSize=10',
        'detailPath': '/monitor/power-board/devices/${deviceId}',
        'deviceHex': '5A4B22170000000100013A985DC02EE01770050000000000000000000000FA281E01000001BB220D0A',
    },
    {
        'id': 'SPD_WAVEFORM',
        'name': 'SPD波形',
        ...
        'variants': [
            {
                'id': 'SPD_WAVEFORM_HEARTBEAT',
                'name': '心跳',
                'bizPath': '',
                'detailPath': '',
                'deviceHex': '5A4B231800000001000138393836303434363130323537303532313831390507EA030F0C1E2DE6340D0A',
            },
            {
                'id': 'SPD_WAVEFORM_SUMMARY',
                'name': '摘要',
                'bizPath': '/monitor/spd-waveform/spd-waveforms?pageNum=1&pageSize=10',
                'detailPath': '/monitor/spd-waveform/devices/${deviceId}',
...
def list_capabilities() -> list[dict[str, Any]]:
    """操作台左侧：一级能力及其二级模块元数据（hidden 能力不展示）。"""
    return [
        {
            "id": cap["id"],
            "name": cap["name"],
