...
使用方法：**每章都对照此表判断等级，没有「本章不承担悬念功能」这个豁免口径**。

硬下限（不达标即补强）：

| 章节定位 | 硬下限 | 说明 |
|---|:---:|---|
| 过渡 / 低压 / 信息整理章 | **≥1 级** | 可以没有强悬念，但不能没有任何往下看的理由 |
| 正文章 | **≥2 级** | 这是下限不是目标值 |
| 关键章 | **≥3 级** | |
| 爆发章 / 卷末 | ≥4 级 | |

下限之上再谈调剂：题材和章节定位决定用哪一类悬念、要不要往上加，但**不能用来往下减**。过渡章判 1 级的同时，仍要有兑现、状态变化和下一步（见 hooks-chapter.md「收一个 / 变一个 / 开一个」）。

> 早先本文件把这三档改成了「只在本章承担悬念功能时判断」「正文章通常 1-2 级」。实测后果：中段大量文书 / 谈判 / 信息交接章会自判为「不承担悬念功能」而整体豁免，连续二十余章无悬念下限，读者反馈为平淡。豁免口径已撤销。
