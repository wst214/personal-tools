...
- 外貌和口头禅是读者记忆锚点，必须写
- **专名必须是中文，不留英文代号**：中文作品里所有故事内专名（人、物、势力、地点、异象、法宝）落笔即用中文定名。构思时顺手用的英文简称——`watcher`、`shadow`、`placeholder`、`A1` 这类——是**未命名状态**，不是名字；它们一旦写进设定或细纲，正文阶段会被当成既有专名照抄，读者看到的就是「那里土气眼还压着没说破的东西，watcher 伏在暗里」这种中英混杂、且和上下文接不上的句子。想不出名字就先写「那个守在暗处的东西」这类中文指称，也不要留英文占位。西幻 / 科幻 / 都市外企等确有外文名的题材，外文名要同时给出中文译名，正文统一用译名，原文只在首次出现时括注一次
