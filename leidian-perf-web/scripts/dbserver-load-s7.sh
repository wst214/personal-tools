#!/usr/bin/env bash
# 在 dbserver 本机造 S7（达梦，清空后全量）
# S7：200 台 × 15 天真 1Hz（std/biz/raw），其它表量级同 S6；写入压测落在大气窗之后
#
# 用法：
#   export DMPASSWORD='Leidian@2026!'
#   chmod +x scripts/dbserver-load-s7.sh
#   ./scripts/dbserver-load-s7.sh
#
# 后台跑（默认，SSH 断开也继续）：
#   ./scripts/dbserver-load-s7.sh
# 前台跟日志：
#   ./scripts/dbserver-load-s7.sh --foreground
#
# 可选环境变量：
#   DMHOST=localhost DMPORT=5236 DMUSER=LEIDIAN_APP DMSCHEMA=PERF
#   DM_HOME=/opt/dmdbms
#   S7_LOAD_LOG=~/s7-load.log
...
STAGE="S7"
FOREGROUND=0
LOG_FILE="${S7_LOAD_LOG:-$HOME/s7-load.log}"
...
if ! grep -q '^  S7:' "${PY_DIR}/config/volume-profiles.yaml"; then
  echo "ERROR: volume-profiles.yaml 中没有 S7，请先同步最新代码" >&2
  exit 1
fi

if ! grep -q 'atmosphere_full_1hz: true' "${PY_DIR}/config/volume-profiles.yaml"; then
  echo "ERROR: S7 需要 atmosphere_full_1hz，请确认已同步含 S7 的 volume-profiles.yaml" >&2
  exit 1
fi
...
if pgrep -f 'run_load\.py load' >/dev/null 2>&1; then
  echo "ERROR: 已有造数进程在跑，请先停掉或等待结束" >&2
  ps aux | grep 'run_load.py load' | grep -v grep || true
  exit 1
fi
...
echo "  log:  ${LOG_FILE}"
echo "  profile: 200台×15天真1Hz，约 2.59亿行/层（std+biz+raw），合计约 7.78 亿"
