#!/usr/bin/env bash
# 在 dbserver 本机造 S5（达梦，清空后全量）
#
# 用法：
#   export DMPASSWORD='Leidian@2026!'
#   chmod +x scripts/dbserver-load-s5.sh
#   ./scripts/dbserver-load-s5.sh
#
# 后台跑（默认）：
#   ./scripts/dbserver-load-s5.sh
# 前台跟日志：
#   ./scripts/dbserver-load-s5.sh --foreground
#
# 可选环境变量：
#   DMHOST=localhost DMPORT=5236 DMUSER=LEIDIAN_APP DMSCHEMA=PERF
#   DM_HOME=/opt/dmdbms
#   S5_LOAD_LOG=~/s5-load.log
...
STAGE="S5"
FOREGROUND=0
LOG_FILE="${S5_LOAD_LOG:-$HOME/s5-load.log}"
...
if ! grep -q '^  S5:' "${PY_DIR}/config/volume-profiles.yaml"; then
  echo "ERROR: volume-profiles.yaml 中没有 S5，请先同步最新代码" >&2
  exit 1
fi
