#!/usr/bin/env bash
# 在 dbserver 本机造 S6（达梦，清空后全量）
#
# 用法：
#   export DMPASSWORD='Leidian@2026!'
#   chmod +x scripts/dbserver-load-s6.sh
#   ./scripts/dbserver-load-s6.sh
#
# 后台跑（默认）：
#   ./scripts/dbserver-load-s6.sh
# 前台跟日志：
#   ./scripts/dbserver-load-s6.sh --foreground
#
# 可选环境变量：
#   DMHOST=localhost DMPORT=5236 DMUSER=LEIDIAN_APP DMSCHEMA=PERF
#   DM_HOME=/opt/dmdbms
#   S6_LOAD_LOG=~/s6-load.log
...
STAGE="S6"
FOREGROUND=0
LOG_FILE="${S6_LOAD_LOG:-$HOME/s6-load.log}"
...
if ! grep -q '^  S6:' "${PY_DIR}/config/volume-profiles.yaml"; then
  echo "ERROR: volume-profiles.yaml 中没有 S6，请先同步最新代码" >&2
  exit 1
fi
