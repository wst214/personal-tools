...
-- === 50km lightning per process (Geography + ST_DWithin，单位米) ===
SELECT p.id,
...
LEFT JOIN biz_lightning_event l
 ...
 AND m.dispatch_room_point IS NOT NULL
 AND DMGEO2.ST_DWithin(
       DMGEO2.ST_GeomToGeog(l.lightning_point),
       DMGEO2.ST_GeomToGeog(m.dispatch_room_point),
       50000)
GROUP BY p.id
ORDER BY p.id
