...
router.beforeEach(async (to, _from, next) => {
    ...
    // DevToolbox / 本地免登：?dev_auto_login=1 → 自动 admin 登录
    if (to.query.dev_auto_login === '1' || sessionStorage.getItem('testhub_dev_auto_login') === '1') {
        sessionStorage.setItem('testhub_dev_auto_login', '1')
        if (!userStore.isAuthenticated) {
            try {
                await userStore.login({ username: 'admin', password: 'admin123' })
            } catch (e) {
                console.error('本地免登失败（请确认 admin/admin123 存在）:', e)
            }
        }
        if (to.query.dev_auto_login === '1') {
            const query = { ...to.query }
            delete query.dev_auto_login
            const path = (!userStore.isAuthenticated)
                ? to.path
                : (to.path === '/login' || to.path === '/' ? '/home' : to.path)
            next({ path, query, replace: true })
            return
        }
        if (userStore.isAuthenticated && (to.path === '/login' || to.meta.requiresGuest)) {
            next('/home')
            return
        }
    }
