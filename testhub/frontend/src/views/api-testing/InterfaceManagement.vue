...
const findCollectionById = (collections, id) => {
  ...
  for (const collection of collections) {
    if (Number(collection.id) === target) return collection
...
const loadAllRequests = async (projectId) => {
  const pageSize = 1000
  let page = 1
  let all = []
  // 后端默认 page_size=20；不拉全量时左侧树会缺接口、集合看起来错乱
  while (true) {
...
const loadRequests = async () => {
  ...
  try {
    const requests = await loadAllRequests(selectedProject.value)
    ...
    requests.forEach(request => {
      const collectionId = typeof request.collection === 'object'
        ? request.collection?.id
        : request.collection
      if (collectionId) {
        // 有关联集合的请求，添加到对应集合下
        const collection = findCollectionById(collections.value, collectionId)
        if (collection) {
