  ...
  <div class="report-list">
    ...
    <el-tabs v-model="activeTab" @tab-change="onTabChange">
      ...
      <el-tab-pane :label="$t('appAutomation.report.suiteReport')" name="suite">
        ...
        <!-- 筛选 -->
        <div class="filters">
          <el-select v-model="suiteProjectFilter" class="filter-item" :placeholder="$t('appAutomation.report.allProjects')" clearable filterable @change="loadSuiteReports">
            <el-option v-for="p in projectList" :key="p.id" :label="p.name" :value="p.id" />
          </el-select>
          <el-input v-model="suiteSearch" class="filter-search" :placeholder="$t('appAutomation.report.searchSuiteName')" clearable @clear="loadSuiteReports" @keyup.enter="loadSuiteReports">
            <template #prefix><el-icon><Search /></el-icon></template>
          </el-input>
          <el-select v-model="suiteStatusFilter" class="filter-status" :placeholder="$t('appAutomation.report.executionStatus')" clearable @change="loadSuiteReports">
        ...
        <el-table :data="suiteReports" v-loading="suiteLoading" border stripe style="width: 100%; min-width: 980px">
          ...
          <el-table-column :label="$t('appAutomation.common.operation')" min-width="220" fixed="right">
            <template #default="{ row }">
      ...
      <el-tab-pane :label="$t('appAutomation.report.caseReport')" name="case">
        ...
        <!-- 筛选 -->
        <div class="filters">
          <el-select v-model="caseProjectFilter" class="filter-item" :placeholder="$t('appAutomation.report.allProjects')" clearable filterable @change="loadCaseReports">
            <el-option v-for="p in projectList" :key="p.id" :label="p.name" :value="p.id" />
          </el-select>
          <el-input v-model="caseSearch" class="filter-search" :placeholder="$t('appAutomation.report.searchCaseDevice')" clearable @clear="loadCaseReports" @keyup.enter="loadCaseReports">
            <template #prefix><el-icon><Search /></el-icon></template>
          </el-input>
          <el-select v-model="caseStatusFilter" class="filter-status" :placeholder="$t('appAutomation.report.executionStatus')" clearable @change="loadCaseReports">
            <el-option :label="$t('appAutomation.common.all')" value="" />
            <el-option :label="$t('appAutomation.status.completed')" value="completed" />
            <el-option :label="$t('appAutomation.report.executionError')" value="error" />
        ...
        <!-- 用例报告列表 -->
        <div class="table-scroll">
        <el-table :data="caseReports" v-loading="caseLoading" border stripe style="width: 100%; min-width: 980px">
...
<style scoped>
.report-list { padding: 4px 2px 10px; }
...
.stat-number { font-size: 22px; font-weight: 700; line-height: 1.4; color: #173b2c; }
.stat-label { font-size: 12px; color: #6b8f7e; }
.filters {
  margin-bottom: 12px;
  padding: 12px 14px;
  background: #fff;
