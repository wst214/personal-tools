...
class AIModelService:
    ...
    @staticmethod
    def _extract_cases_from_reasoning(reasoning: str) -> str:
        """从推理模型 reasoning_content 中尽量提取 Markdown 用例表格。"""
        if not reasoning:
            return ''
        text = reasoning.strip()
        # 必须命中表头行，避免把思考过程里零星的 | / 用例ID 讨论当成正文
        header = re.search(
            r'(?m)^\|[^\n]*用例ID[^\n]*\|[^\n]*$',
            text,
        )
        if header:
            body = text[header.start():].strip()
            # 至少再有一条用例数据行
            if re.search(r'(?m)^\|\s*[A-Za-z][A-Za-z0-9_]*_\d+\s*\|', body):
                return body
        # 无表头时：连续 Markdown 用例行块
        rows = re.findall(
            r'(?m)^\|\s*[A-Za-z][A-Za-z0-9_]*_\d+\s*\|[^\n]*\|$',
            text,
        )
        if len(rows) >= 3:
            first = text.find(rows[0])
    ...
    async def call_openai_compatible_api(
        ...
        # 推理模型会先消耗 thinking tokens，过小易导致正文为空
        configured_tokens = max_tokens if max_tokens is not None else (config.max_tokens or 4096)
        actual_max_tokens = max(int(configured_tokens), 16384)
    ...
    async def call_openai_compatible_api_stream(
        ...
        while continuation_count <= MAX_CONTINUATIONS:
            ...
            chunk_content_buffer = ""  # 本次请求生成的完整正文
            reasoning_buffer = ""  # 推理模型思考过程（content 为空时兜底）
            finish_reason = None
            ...
            try:
                        ...
                        async for line in response.aiter_lines():
                            ...
                            if line.startswith('data: '):
                                ...
                                try:
                                    ...
                                    # 推理模型：正文在 content；思考在 reasoning_content
                ...
                # content 为空但 reasoning 里已有用例表格时，作为兜底输出
                if not chunk_content_buffer.strip() and reasoning_buffer.strip():
                    fallback = AIModelService._extract_cases_from_reasoning(reasoning_buffer)
                    if fallback:
                        logger.warning(
                            f"流式 content 为空，已从 reasoning_content 提取用例正文 (len={len(fallback)})"
                        )
                        chunk_content_buffer = fallback
                        if callback:
                ...
                if finish_reason == 'length':
                    logger.warning(
                        ...
                        f" content_len={len(chunk_content_buffer)}, reasoning_len={len(reasoning_buffer)}"
                    )
                    continuation_count += 1

                    # 将本次生成的内容作为 assistant 回复加入历史
                    # 注意：如果之前已经有assistant消息，需要追加内容而不是新增消息
                    assistant_text = chunk_content_buffer or reasoning_buffer
                    if current_messages[-1]['role'] == 'assistant':
                        current_messages[-1]['content'] += assistant_text
                    else:
                        current_messages.append({"role": "assistant", "content": assistant_text})

                    # 只有当上一条不是user的续写指令时，才添加新的 user 指令
                    # 防止多次续写时堆叠重复的 user 指令
                    if current_messages[-1]['role'] != 'user':
                        current_messages.append(
                            {"role": "user", "content": "请继续输出剩余的 Markdown 用例表格，不要重复已输出的部分，紧接着上文继续。直接输出表格，不要输出思考过程。"})

                    continue
                else:
                    logger.info(
                        f"流式生成正常结束 (finish_reason={finish_reason}) "
                        f"content_len={len(chunk_content_buffer)} reasoning_len={len(reasoning_buffer)}"
                    )
                    break
    ...
    async def generate_test_cases_stream(
        ...
        if not (full_content and full_content.strip()):
            logger.warning(f"任务 {task.task_id} 流式结果为空，回退非流式生成")
            try:
                response = await AIModelService.call_openai_compatible_api(
                    task.writer_model_config,
                    messages,
                    max_tokens=max(int(task.writer_model_config.max_tokens or 4096), 16384),
                )
                full_content = (
                ...
                if not full_content:
                    # 部分推理模型把正文放在 reasoning / reasoning_content
                    msg = (response.get('choices') or [{}])[0].get('message', {}) or {}
                    full_content = AIModelService._extract_cases_from_reasoning(
                        msg.get('reasoning_content') or msg.get('reasoning') or ''
                    )
                if full_content and callback:
                    await callback(full_content)
                logger.info(f"非流式回退完成: 字符数={len(full_content)}")
            except Exception as fallback_error:
                logger.error(f"非流式回退失败: {fallback_error}")

        # 统计生成的用例数量
        case_count = len(re.findall(r'[A-Za-z]+_\d+', full_content)) + full_content.count('测试用例')
