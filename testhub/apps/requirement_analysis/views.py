                        ...
                        def execute_task():
                            try:
                                ...
                                try:
                                                ...
                                                try:
                                                    ...
                                                    try:
                                                        ...
                                                        if revised_cases and len(revised_cases) > 0:
                                                            ...
                                                            # 改进返回为空：优先保留流式内容，否则回退原始生成结果
                                                            if not (task.final_test_cases and task.final_test_cases.strip()):
                                                                sorted_cases = AIModelService.sort_test_cases_by_id(
                                                                    generated_cases)
                                                                task.final_test_cases = AIModelService.renumber_test_cases(
                                                                    sorted_cases)
                                                                logger.warning(
                                                                    f"任务 {task.task_id} 改进返回为空且流式内容为空，回退原始用例 (长度: {len(task.final_test_cases)})")
                                                            else:
                                                                logger.warning(
                                                                    f"任务 {task.task_id} 改进返回为空，使用流式回调保存的内容 (长度: {len(task.final_test_cases)})")
                                    ...
                                    # 评审改进可能清空 final 却未写回：完成前用 generated 兜底
                                    if not (task.final_test_cases and task.final_test_cases.strip()) and task.generated_test_cases:
                                        sorted_cases = AIModelService.sort_test_cases_by_id(task.generated_test_cases)
                                        task.final_test_cases = AIModelService.renumber_test_cases(sorted_cases)
                                        logger.warning(
                                            f"任务 {task.task_id} 完成时 final 为空，已用 generated 回填 (长度: {len(task.final_test_cases)})")

                                    # 生成与最终结果都为空：不应标记为已完成
                                    has_cases = bool(
                                        (task.final_test_cases and task.final_test_cases.strip())
                                        or (task.generated_test_cases and task.generated_test_cases.strip())
                                    )
                                    if not has_cases:
                                        task.status = 'failed'
                                        task.progress = 100
                                        ...
                                        task.error_message = (
                                            task.error_message
                                            or '生成结果为空（可能因模型超时、限流或并发生成失败），请重试'
                                        )
                                        task.save(update_fields=[
                                            'status', 'progress', 'completed_at',
                                            'final_test_cases', 'error_message'
                                        ])
                                        logger.error(f"任务 {task.task_id} 结果为空，已标记失败")
                                    else:
                                        task.status = 'completed'
                                        task.progress = 100
                                        task.completed_at = timezone.now()
                                        task.save(update_fields=[
                                            'status', 'progress', 'completed_at', 'final_test_cases'
                                        ])
