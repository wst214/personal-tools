...
const ICON = svg('<path d="M5 4l14 8-14 8z"/>');
const STOP_ICON = svg('<rect x="6" y="6" width="12" height="12" rx="1.5"/>');
const FOLDER_ICON = svg('<path d="M3 7.5a2 2 0 0 1 2-2h3.5l2 2H19a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/>');
...
const webStore = {
  _key: 'devtool-deploy-demo',
  _seededKey: 'devtool-deploy-seeded',
  _listeners: [],
  _running: false,
  _all() {
...
// 惰性取 store：window.toolbox 可能在模块加载后才由 tauri-shim 注入
function currentStore() {
  return (window.toolbox && window.toolbox.deploy) ? window.toolbox.deploy : webStore;
}
function currentOnOutput() {
  return (window.toolbox && window.toolbox.onDeployOutput) ? window.toolbox.onDeployOutput : ((cb) => webStore.onOutput(cb));
}
let store = currentStore();
let onOutput = currentOnOutput();
...
export const deployTool = {
  ...
  category: '系统',
  icon: ICON,
  keywords: 'deploy 部署 任务 命令 构建 打包 docker',
  desc: '任务清单 + 命令顺序执行，支持参数化变量',
  state: {
    tasks: [],
    selected: null,
    form: null,
  ...
  async render(container) {
    ...
    // 刷新 store/onOutput（tauri-shim 注入可能晚于模块加载）
    store = currentStore();
    onOutput = currentOnOutput();
    await this.refresh();
    this.paint();
    // 默认选中第一个任务（或恢复仍存在的上次选中）
    if (this.state.tasks.length) {
      const keep = this.state.selected && this.state.tasks.some((t) => t.id === this.state.selected);
      await this.selectTask(keep ? this.state.selected : this.state.tasks[0].id);
    } else {
  ...
  paintRight() {
    ...
    const paramField = f.params && f.params.length
      ? el('div', { class: 'params' }, ...f.params.map(renderParam))
      : null;

    const commandCount = (f.commands || '').split('\n').filter((s) => s.trim()).length;
    const envCount = (f.env || '').split('\n').filter((s) => s.trim()).length;
    ...
    const form = el(
      ...
      el(
        'div',
        { class: 'deploy-form-head' },
        el('div', { class: 'deploy-form-title', text: isNew ? '新建任务' : '编辑任务' }),
        el(
          'div',
      ...
      el(
        'details',
        { class: 'deploy-advanced', open: isNew ? true : null },
        el(
          'summary',
          {},
          el('span', { text: '高级配置' }),
          el('span', { class: 'advanced-meta', text: `${commandCount} 条命令 · ${envCount} 个变量` }),
        ),
        el(
          'div',
          { class: 'advanced-grid' },
  ...
  // 增量追加一行输出（避免高频日志流下全量重建 DOM）
  appendOutputLine(msg) {
    if (!this.outputEl) return;
    if (!this.state.output.length) this.outputEl.innerHTML = ''; // 首次清掉占位
    const cls = 'out-line ' + (msg.type || 'out');
    this.outputEl.append(el('div', { class: cls, text: msg.text }));
    this.outputEl.scrollTop = this.outputEl.scrollHeight;
  },

  _syncRunBtn() {
    const actions = this.root?.querySelector?.('.deploy-actions');
    if (!actions?.firstElementChild) return;
    const next = this.state.running
      ? el('button', { class: 'btn danger', onclick: () => this.stop() }, '停止')
      : el('button', { class: 'btn primary', onclick: () => this.run() }, '运行');
    actions.firstElementChild.replaceWith(next);
  },

  onOutput(msg) {
    // start/done/cancelled 只切换运行按钮，避免整表单重建丢焦点
    if (msg.type === 'start') {
    ...
    if (msg.type === 'done' || msg.type === 'cancelled') {
      ...
      this.appendOutputLine(this.state.output[this.state.output.length - 1]);
      this._syncRunBtn();
      return;
    }
    if (msg.type === 'failed') {
    ...
    // 防爆：保留最近 1500 条
    if (this.state.output.length > 1500) {
      this.state.output = this.state.output.slice(-1500);
      // 超限后重建一次
      this.paintOutput();
      return;
    }
    this.appendOutputLine(msg);
  },
  ...
  async run() {
    ...
    const ctx = {
      cwd: task.cwd || '',
      toolbox: window.toolbox?.paths?.toolboxRoot || 'D:\\mytools\\dev-toolbox',
    };
