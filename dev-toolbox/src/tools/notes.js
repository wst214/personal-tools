...
function normalizeNote(n, index = 0) {
  ...
  return {
    ...
    body,
    ts: n?.ts || n?.createdAt || now,
    updatedAt: n?.updatedAt || n?.ts || now,
  };
}
...
export const notesTool = {
  ...
  render(c) {
    ...
    const bodyCheck = el('input', {
      type: 'checkbox',
      checked: 'checked',
      onchange: (e) => { searchBody = e.target.checked; runSearch(); },
    });
    const sortSelect = el('select', {
      class: 'select note-sort',
      onchange: (e) => { sortBy = e.target.value; renderTree(); },
    }, [
      el('option', { value: 'updated', text: '最近修改' }),
      el('option', { value: 'created', text: '创建时间' }),
      el('option', { value: 'title', text: '标题' }),
    ]);
    ...
    const dirBar = el('div', { class: 'note-dir-bar' }, [
      el('span', { class: 'note-dir-icon', text: '📁' }),
      dirLabel,
      defaultBadge,
      activeHint,
    ...
    const noteLibrary = el('aside', { class: 'note-library' }, [
      search,
      el('div', { class: 'note-library-controls' }, [
        el('label', { class: 'check note-body-check' }, [bodyCheck, el('span', { text: '搜索正文' })]),
        sortSelect,
      ]),
      el('div', { class: 'note-library-actions' }, [
        btn('新建笔记', createNote, { variant: 'ghost' }),
        btn('新建文件夹', createDir, { variant: 'ghost' }),
        btn('复制', duplicateNote, { variant: 'ghost' }),
        btn('删除', deleteNote, { variant: 'ghost' }),
      ]),
      noteList,
    ]);
    ...
    const noteShell = el('div', { class: 'note-workbench' }, [
      el('div', { class: 'note-workbench-head' }, [
        el('h1', { text: '随手记' }),
        el('div', { class: 'note-mode-switch' }, [modeBtns.edit, modeBtns.split, modeBtns.preview, outlineToggle]),
      ]),
      dirBar,
      el('div', { class: 'note-workbench-body' }, [
        ...
        el('section', { class: 'note-editor-shell' }, [
          el('div', { class: 'note-editor-toolbar' }, [
            el('button', { class: 'note-icon-btn', type: 'button', title: '加粗', text: 'B', onclick: () => wrapSelection('**') }),
            el('button', { class: 'note-icon-btn', type: 'button', title: '斜体', text: 'I', onclick: () => wrapSelection('*') }),
            el('button', { class: 'note-icon-btn', type: 'button', title: '行内代码', text: '<>', onclick: () => wrapSelection('`') }),
            el('button', { class: 'note-icon-btn', type: 'button', title: '标题', text: 'H', onclick: () => prefixSelection('# ') }),
            el('button', { class: 'note-icon-btn', type: 'button', title: '列表', text: '≡', onclick: () => prefixSelection('- ') }),
            el('span', { class: 'note-toolbar-spacer' }),
            el('select', { class: 'select note-toolbar-select' }, [el('option', { text: 'Markdown' })]),
            el('select', { class: 'select note-toolbar-select small' }, [14, 15, 16, 18].map((n) => el('option', { text: String(n), value: String(n), selected: n === 14 ? 'selected' : null })) ),
            el('select', { class: 'select note-toolbar-select small' }, ['1.0×', '1.1×', '1.2×'].map((n) => el('option', { text: n })) ),
            el('button', { class: 'note-icon-btn', type: 'button', title: '复制正文', text: '⧉', onclick: copyCurrent }),
          ]),
    ...
    editor.addEventListener('input', () => {
      ...
      if (!title.value.trim()) note.title = firstTitle(note.body) || 'Untitled Note';
      note.updatedAt = Date.now();
      renderPreview();
      renderTree();
      updateFooter('保存中...');
    ...
    function dirRow(n, depth) {
      ...
      const head = el('button', {
        ...
        el('span', { class: 'note-tree-icon', text: '📁' }),
        el('span', { class: 'note-tree-name', text: n.name }),
        el('span', { class: 'note-tree-add', title: '在此新建笔记', text: '＋', onclick: (e) => { e.stopPropagation(); setActive(n.path); expanded.add(n.path); createNote(); } }),
      ]);
    ...
    function fileRow(n, depth) {
      return el('button', {
        ...
        el('span', { class: 'note-list-main' }, [
          ...
          el('span', { class: 'note-list-meta', text: new Date(n.updatedAt || n.ts).toLocaleString('zh-CN') }),
        ]),
      ]);
    }
    ...
    async function pickDir() {
      ...
      setDir(r.dir); tree = r.tree || []; if (r.defaultDir) defaultDir = r.defaultDir; setActive(''); expanded.clear(); expandTopLevel();
    ...
    (async () => {
      if (isDesktop()) {
        ...
        if (r.ok) { setDir(r.dir); tree = r.tree || []; if (r.defaultDir) defaultDir = r.defaultDir; expandTopLevel(); }
        else { setDir(''); tree = []; }
      } else {
        setDir('');
