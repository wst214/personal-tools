...
(function () {
  function initTauriToolbox() {
    ...
    window.toolbox = {
      ...
      diffScanDir: (dir) => invoke('diff_scan_dir', { dir }).catch((e) => ({ ok: false, error: String(e) })),
      diffReadText: (path, maxBytes) => invoke('diff_read_text', { path, maxBytes: maxBytes ?? null }).catch((e) => ({ ok: false, error: String(e) })),
      ...
      ssh: {
        ...
        localOpen: (shell) => invoke('local_shell_open', { shell }).catch((e) => ({ ok: false, error: String(e) })),
      },
      sftp: {
